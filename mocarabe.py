...
parser.add_argument('-dfg', metavar='dfg dir', required=True,
                    type=str, help='path to dfg dir, contains .hgr file')
parser.add_argument('-II', metavar='initiation interval', required=True,
                    type=int, default=1, help="how often we can input new data into the system")
parser.add_argument('-C', metavar='channel width', type=int,
                    default=1, help="physical channel width for NoC")
parser.add_argument('--log', metavar='log', type=str,
                    default='log.csv', help="log, useful for running experiments")
parser.add_argument('--tag', metavar='tag for logging',
                    type=str, default="-", help="tag for logging")
parser.add_argument('--place_time', metavar='placement duration (minutes)',
                    type=float, default=0.01, help="max duration for cgra placement")
# parser.add_argument('-T', metavar='schedule length', type=int, default = 0, help='schedule length (optional upper bound)')
parser.add_argument(
    '--unroll', help='DFG unroll factor, default is 1, 0 for automatic unroll (max is II)', type=int, default=1)
parser.add_argument('--sched_method', metavar='scheduling_method',
                    type=str, default='PF', help="sched method, either ILP or PF")
...
parser.add_argument('--noc_pipe', metavar='noc pipelining',
...
if schedule_length != II:
    warnings.warn(
        f"{bcolors.WARNING}T != II: this is experimental{bcolors.ENDC}")
if io_diffusion > II:
    warnings.warn(
        f"{bcolors.WARNING}iod > II: trying to pack more input nodes in a PE than the modulo schedule can handle{bcolors.ENDC}")
if arith_diffusion > II:
    warnings.warn(
        f"{bcolors.WARNING}ard > II: trying to pack more operations in a PE than the modulo schedule can handle{bcolors.ENDC}")

assert (os.path.exists(dfg_dir)
        ), f"{os.path.realpath(dfg_dir)} does not exist.  Have you run gcc?"
assert (sched_method == 'ILP' or sched_method ==
        'PF'),  "sched_method must be either 'ILP' or 'PF' "
...
shutil.copy(os.path.join(dfg_dir, benchmark_name+'.hgr'),
            file_helper.hgr_dir + benchmark_name + '.hgr')
...
for rtl_file in ['pe_mux_2_input.sv', 'pe_srl.v', 'pe_2_input.sv', 'pe_mux_3_input.sv', 'SRL16E.v', 'SRLC32E.v', 'SRL64.v', 'torus_switch.sv', 'mocarabe.sv', 'mocarabe.h']:
    shutil.copy(os.path.join('rtl', 'rtl', rtl_file),
                os.path.join(file_helper.proj_dir, 'rtl', rtl_file))
...
if unroll_factor == 0:
    ...
    io_operators = collections.Counter(
        dataflow_hgraph.extract_node_io_operators().values())
    if '+' not in arith_oprtrs:
        arith_oprtrs['+'] = 0
    if '*' not in arith_oprtrs:
        arith_oprtrs['*'] = 0
...
write_dot(dataflow_hgraph.to_graph(), os.path.join(
    file_helper.dot_dir, benchmark_name + ".dot"))


''' PE Allocation '''

Nx, Ny, device_map = pe_alloc.ClosestFactorsAllocator().allocate_pes(
    dataflow_hgraph, II, io_diffusion, arith_diffusion)
print(f'Nx={Nx}, Ny={Ny}')
...
partition_filename = file_helper.partition_dir + \
    benchmark_name + "-K" + str(Nx*Ny) + "-U" + str(unroll_factor)

dfg_node_to_pe_xy_map_, pe_operators = placr.IlpAndSimulatedAnnealingPlacer().place(dataflow_hgraph, Nx*Ny,
                                                                                    num_partitions_given_to_operator, partition_filename + '.sol', II, placement_time_delta, Nx, Ny, file_helper.log_dir)

# serialize placement
file_helper.write_list_to_file(
    file_helper.placement_dir + 'dfg_node_to_pe_xy.json', dfg_node_to_pe_xy_map_)

# serialize placed netlist (scheduler input)
serialized_placed_netlist = placr.serialize_placed_netlist(
    dataflow_hgraph, dfg_node_to_pe_xy_map_, True)
placed_netlist = placr.create_placed_netlist(
    dataflow_hgraph, dfg_node_to_pe_xy_map_, True)
...
file_helper.schedule_filepath = file_helper.schedule_dir + benchmark_name + \
    '-Nx%d-Ny%d-C%d-num_nets%d-T%d' % (Nx, Ny,
                                       C, num_nets, schedule_length) + '.sol'

layout = ''
device = Device(Nx, Ny, C, schedule_length, schedule_length, IO_I, IO_O, layout,
                pe_pipelining_stages, noc_pipelining_stages, unroll_factor, num_nets, II)
if sched_method == 'ILP':
    boundingBoxEnabled = False

    scheduled_netlist = schedulr.IlpScheduler().run_scheduling_with_timeout(
    ...
    if scheduled_netlist != None:
        ...
        RTLGenerator.generate_and_write_noc_mux_memories(
            file_helper.rtl_dir, h, v, enter, exit_, Nx, Ny, C, num_nets, T, noc_pipelining_stages)

        ''' verify scheduled netlist channel utilization '''
        utilization_factor = scheduled_netlist.physical_channel_utilization_factor(
            device.Nx, device.Ny, device.physical_channels, T)
        unused_xy = scheduled_netlist.unused_xy(
            device.Nx, device.Ny, device.physical_channels, T)
        ...
        file_helper.write_string_to_file(
            verilog_header_filename, verilog_header)

        resource_graph = ResourceGraph()
        resource_graph.create(device)

        net_paths = RTLGenerator.get_net_path_nodes(
            dataflow_hgraph, resource_graph, device, h, v, enter, exit_)
        op_addresses, op_port_select, asserts_string, latency6 = RTLGenerator.generate_and_write_pe_memories(
            file_helper.proj_dir, resource_graph, net_paths, dataflow_hgraph, dfg_node_to_pe_xy_map_, enter, exit_, NUM_OPERANDS, IO_I, II, device)
    ...
    warnings.warn(
        f"{bcolors.WARNING} PathFinder scheduler not maintained to the same level as ILP scheduler")
    file_helper.tag = tag
