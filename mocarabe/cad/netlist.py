...
class Netlist(DirectedHypergraph):
    ...
    def serialize_df_hypergraph(self, path: str) -> None:
        """
        ...
        with open(path, "r") as fp:
            for line_count, line in enumerate(fp):
                if line_count == 0:
                    ...
                    node_id, label = (
                        node_line.split(":")[0].strip(),
                        node_line.split(":")[1].strip(),
                    )
    ...
    def unroll_dfg(self, unroll_factor: int):
        ...
        for loop_num in range(unroll_factor):
            for h_edge_id in ordered_hyperedge_id_list:
                ...
                self.add_hyperedge([loop_src], loop_sinks)

        assert len(self.get_node_set()) == unroll_factor * len(initial_node_set)
        assert len(self.get_hyperedge_id_set()) == unroll_factor * len(
            initial_hyperedge_id_set
        )
        ...
        for hyperedge_id in self.ordered_hyperedge_id_list():
            ...
            for head_node in hyperedge["head"]:
                ...
                if maxDepth[head_node] - maxDepth[tail_node] > 1:
                    retiming_registers = (
                        retiming_registers
                        + (maxDepth[head_node] - maxDepth[tail_node])
                        - 1
                    )
        ...
        for hyperedge_id in list(self.ordered_hyperedge_id_iterator()):
            ...
            if (hyperedge["tail"]) == start:
                for neighbour in hyperedge["head"]:
                    ...
                    neighbour_depth.update(
                        self.__get_successor_depth_levels(
    ...
    def get_parent_nodes(self, node):
        ...
        for hyperedge_id in parent_hyperedges:
            ...
            assert len(hyperedge["tail"]) == 1, (
                "Hyperedges can only have one tail (source) node"
            )
