...
def partition_with_ilp(
    ...
    for partition in range(K):
        if partition_operator(operator_ranges, partition) in ["*", "+"]:
            m.addConstr(
                quicksum(x[v, partition] for v in range(V)) >= MIN_ARITH_PARTITION_SIZE
            )  # TODO can update
    #
    ...
    for partition in range(K):
        ...
        if partition_operator(operator_ranges, partition) == "*":
            ...
            m.addConstr(
                quicksum(x[v, partition] for v in range(V)) <= MAX_ARITH_PARTITION_SIZE
            )
    ...
    for partition in range(K):
        if partition in operator_ranges["IN"]:
            m.addConstr(
                quicksum(x[v, partition] for v in range(V)) >= MIN_IO_PARTITION_SIZE
            )  # TODO can update
    ...
    for partition in range(K):
        ...
        if partition in operator_ranges["IN"]:
            ...
            m.addConstr(
                quicksum(x[v, partition] for v in range(V)) <= MAX_IN_PARTITION_SIZE
            )
        # >= operator_ranges['IO'][0] and partition <=  operator_ranges['IO'][1]:
        elif partition in operator_ranges["OUT"]:
            # MAX_IO_PARTITION_SIZE = MAX_IO_PARTITION_SIZE + 8
            m.addConstr(
                quicksum(x[v, partition] for v in range(V)) <= MAX_OUT_PARTITION_SIZE
            )
