...
class Pathfinder:
    def __init__(
        self, resource_graph, connections, MAX_ITER_pf=20, MAX_ITER_tp=10, VERBOSE=False
    ):
    ...
    def pathfinder_ts(self, starting_slot, list_of_connections):
        ...
        while (
            self.shared_channel_slot_pairs_exist()
            and iteration_count < self.MAX_ITER_pf
                ...
                for sink in sink_nodes:  # "LOOP UNTIL ALL SINKS HAVE BEEN FOUND"
                    ...
                    while not any(i in sink_switches for i in sink_shortest_path):
                        ...
                        for fanout_n in self.G.successors(node_m):
                            ...
                            if (
                                connection.id == 6
                                and self.G.node_attributes_type[fanout_n]
                                == ResourceType.PE_OUT_SWITCH
                            ):
                                print(
                            ...
                            if (
                                fanout_n not in self.G.pe_in_nodes
                                and fanout_n not in self.G.pe_nodes
                            ) or (fanout_n_attr[0], fanout_n_attr[1]) == sink_xy:
                            ...
                            shortest_path.append(
                                self.G.pe_out_port[
                                    (
                                        ...
                                        connection.source_xy[0],
                                        connection.source_xy[1],
                                        io_o,
                                        t,
                                    )
                                ]
                    ...
                    for node_in_path in reversed(connection.net_path):
                        ...
                        if node_in_path in self.G.pe_out_switch.values():
                            ...
                            node_in_path in self.G.h_noc.values()
                            or node_in_path in self.G.v_noc.values()
                        ):
                            expansion_list.push(node_in_path, 0)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="path to *net file to run pathfinder directly",
    )
    parser.add_argument(
        "-C",
        metavar="channel count",
        required=True,
        type=int,
        help="exact # of physical channels",
    )
    parser.add_argument(
        "-Nx",
        metavar="x-dimension of 2D chip",
        required=True,
        type=int,
        help="the x-size of the network",
    )
    parser.add_argument(
        "-Ny",
        metavar="y-dimension of 2D chip",
        required=True,
        type=int,
        help="the y-size of the network",
    )
    parser.add_argument(
        "-T",
        metavar="schedule length",
        required=True,
        type=int,
        default=0,
        help="schedule length (optional upper bound)",
    )
    ...
    device = Device(
        Nx, Ny, C, T, IO_I, IO_O, layout, pe_pipelining_stages, noc_pipelining_stages
    )
