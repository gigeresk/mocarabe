...
def allocate_pe_with_strategy(
    dataflow_hgraph,
    II,
    io_diffusion,
    arith_diffusion,
    device_constraints=[],
    strategy="closest_factors",
):
    """Call the specified PE allocation strategy"""

    if strategy in ["closest_factors", [], None]:
        return ClosestFactorsAllocator().allocate_pes(
            dataflow_hgraph, II, io_diffusion, arith_diffusion
        )
    elif strategy == "constrained_closest_factors":
        return ConstrainedClosestFactorsAllocator().allocate_pes(
            dataflow_hgraph, II, io_diffusion, arith_diffusion, device_constraints
        )
    else:
        raise ValueError(
            f"strategy must have a valid value, not {strategy} of type {type(strategy)}"
        )
