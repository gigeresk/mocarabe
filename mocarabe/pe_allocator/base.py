...
class PEAllocatorStrategy(metaclass=abc.ABCMeta):
    ...
    def _paint_device_with_op_constraints(device_constraints):
        # [[ '' for y in range(MAX_DEVICE_Y_DIM)] for x in range(MAX_DEVICE_X_DIM)]
        device_operator_allocation = np.ndarray(
            (PEAllocatorStrategy.MAX_DEVICE_X_DIM, PEAllocatorStrategy.MAX_DEVICE_Y_DIM), dtype=object)
