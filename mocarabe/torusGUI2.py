...
class MainApplication(tk.Frame):
    def __init__(self, parent=None):
        ...
        enter = [
            [[[0 for t in range(T)] for p in range(P)] for y in range(Ny)]
            for x in range(Nx)
        ]
        exit = [
            [[[0 for t in range(T)] for p in range(P)] for y in range(Ny)]
        ...
        def drawCanvas(time):
            ...
            for x in range(0, Nx):
                for y in range(0, Ny):
                    ...
                    w.create_text(
                        (x1 + x2) / 2, (y1 + y2) / 2, text=str(x) + "," + str(y)
                    )
                ...
                for y in range(0, Ny):
                    for c in range(0, C):
                        if C > 1:  # increment, starting at a quarter of the switch
                            ...
                            y1 = y2 = lowerBound + ((c) * 0.5 / float(C - 1)) * (
                                stepy - offsety
                            )
                        else:  # centre
                ...
                for y in range(0, Ny):
                    for c in range(0, C):
                        if C > 1:  # increment, starting at a quarter of the switch
                            ...
                            x1 = x2 = lowerBound + ((c) * 0.5 / float(C - 1)) * (
                                stepx - offsetx
                            )
                        else:  # centre
