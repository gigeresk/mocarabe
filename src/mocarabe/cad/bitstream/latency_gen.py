...
def generate_unadjusted_global_timing(
    head_node,
    this_edge,
    head_node_enter_noc_cycle,
    global_node_execute_cycle,
    global_node_enter_cycle,
    node_and_edge_port,
    edge_and_fanout_relative_travel_time,
    enter,
    exit_,
    C,
    T,
    ...
    if tail_pe_xy[0] == head_pe_xy[0] and tail_pe_xy[1] == head_pe_xy[1]:  # self edge
        ...
        curr_enter_cycle = (
            global_node_execute_cycle[head_node]
            - 1
            - cycles_on_noc
            - REGISTER_OPERAND_CYCLE
            - OP_LOAD_CYCLE
        )
        edge_and_fanout_relative_travel_time[int(this_edge[1:]) - 1][head_node] = (
            cycles_on_noc
        )
    else:
        # not self edge
    ...
    if tail_node in global_node_enter_cycle:
        global_node_enter_cycle[tail_node] = min(
            curr_enter_cycle, global_node_enter_cycle[tail_node]
        )
    else:
        global_node_enter_cycle[tail_node] = curr_enter_cycle

    global_node_execute_cycle[tail_node] = (
        curr_enter_cycle - device.pe_pipelining_stages
    )
...
def get_node_values(input_values, dataflow_graph):
    ...
    while len(dataflow_graph.get_node_set()) != len(node_values):
        ...
        for hyperedge_id in dataflow_graph.ordered_hyperedge_id_iterator():
            ...
            if tail_id not in node_values:
                continue  # node already computed
            ...
            for head_id in heads:
                ...
                if head_id not in partial_results:
                    ...
                    head_id in partial_results
                    and partial_results[head_id][0] != tail_id
                ):
                    # already computed only the other half of the partial sum
                    if head_label == "*":
                        node_values[head_id] = (
                            node_values[tail_id] * partial_results[head_id][1]
                        )
                    elif head_label == "+":
                        node_values[head_id] = (
                            node_values[tail_id] + partial_results[head_id][1]
...
def generate_and_write_pe_memories(
    ...
    edge_and_fanout_relative_travel_time = [0] * len(
    ...
    for leaf_node in leaf_nodes:
        ...
        for leaf_edge in leaf_edges:
            leaf_exit_noc_cycle = cycle_for_pe_op(
                leaf_node_pe_xy, int(leaf_edge[1:]) - 1, exit_, C, T
            )

            global_node_execute_cycle[leaf_node] = (
                leaf_exit_noc_cycle - pe_pipelining_stages
            )
            (
                global_node_execute_cycle,
                global_node_enter_cycle,
        ...
        print(global_node_execute_cycle)
        global_node_execute_cycle, global_node_enter_cycle, latency = (
            adjust_global_timing(
                # TODO: have these be parameters
                global_node_execute_cycle,
                global_node_enter_cycle,
            )
        )
    ...
    while current_node not in leaf_nodes:
        print(
            f"executing node {current_node} ({dataflow_graph.get_node_attributes(current_node)['label']}) at {placement_result[current_node]} at cycle {int(global_node_execute_cycle[current_node])}"
        )
        ...
        # import pdb; pdb.set_trace()
        for critical_path_candidate in dataflow_graph.get_hyperedge_attribute(
            next_hyperedge, "head"
        ):
            try:
                if (
                    global_node_execute_cycle[critical_path_candidate]
                    < min_cycle_node[1]
                ):
                    min_cycle_node = (
