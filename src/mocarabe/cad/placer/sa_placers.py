        ...
        for node_i in dataflow_hypergraph.ordered_node_id_list():
            for node_j in dataflow_hypergraph.ordered_node_id_list():
                ...
                if i_is_topo_precedent:  # if i comes before j in the dataflow graph
                    if (
                        i_node_x > j_node_x and i_node_y > j_node_y
                    ):  # if i is dominated by j
                        state[j_node_partition], state[i_node_partition] = (
                            state[i_node_partition],
                            state[j_node_partition],
                        )
...
def debug_energy(
    dataflow_hypergraph, partitioned_netlist, dfg_v_to_partition_id, state, Nx, Ny
):
    e = 0
    ...
    def __init__(
        ...
        for source in range(Nx * Ny):
            for dest in range(Nx * Ny):
                tmp = self.memoized_torus_distance[source][dest] = (
                    torus_min_distance_xy(source, dest, Nx, Ny)
                )
                self.memoized_torus_distance_squared[source][dest] = (tmp[0] + 1) * (
                    tmp[1] + 1
                )
...
class QuadraticWirelengthAnnealingPlacer(AbstractAnnealingPlacer):
    def __init__(
        ...
        super(QuadraticWirelengthAnnealingPlacer, self).__init__(
            state,
            dataflow_hypergraph,
            partitioned_netlist,
            Nx,
    ...
    def energy(self):
        ...
        for (
            source_partition,
            dest_partition,
        ) in self.memoized_source_pe_and_dest_partitions:
...
class LinearWirelengthAnnealingPlacer(AbstractAnnealingPlacer):
    def __init__(
        ...
        super(LinearWirelengthAnnealingPlacer, self).__init__(
            state,
            dataflow_hypergraph,
            partitioned_netlist,
            Nx,
    ...
    def energy(self):
        ...
        for (
            ...
            e += sum(
                self.memoized_torus_distance[self.state[source_partition]][
                    self.state[dest_partition]
                ]
            )
        return e
