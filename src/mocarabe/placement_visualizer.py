...
class MainApplication(tk.Frame):
    ...
    def __init__(
        ...
        canvas.create_rectangle(
            0,
            0,
            Nx * self.stepx + 3 * offsetx,
            (Ny) * self.stepy + 3 * offsetx,
            fill="white",
        )
        if energy != 0:
            canvas.create_text(
                (Nx * self.stepx + 3 * offsetx) // 2, 20, text=str(energy)
            )
        # PE box at every (x,y)
        for x in range(0, Nx):
            for y in range(0, Ny):
...
def visualize_placement(
    dataflow_hypergraph, Nx, Ny, dfg_node_to_xy, output_path, energy=0, kill=True
):
    root = tk.Tk()
    root.title("Placement Visualizer")
    root.configure(background="black")
    MainApplication(
        Nx, Ny, dataflow_hypergraph, dfg_node_to_xy, output_path, energy, kill, root
