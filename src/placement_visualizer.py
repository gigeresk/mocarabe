...
class MainApplication(tk.Frame):
    ...
    def __init__(self, Nx, Ny, dataflow_hypergraph, dfg_node_to_xy, output_path, energy=0, kill=True, parent=None):
        ...
        canvas.create_rectangle(
            0, 0, Nx*self.stepx+3*offsetx, (Ny)*self.stepy+3*offsetx, fill="white")
        if energy != 0:
            canvas.create_text((Nx*self.stepx+3*offsetx) //
                               2, 20, text=str(energy))
        # PE box at every (x,y)
        for x in range(0, Nx):
            for y in range(0, Ny):
                # PE box
            ...
            for sink in sinks:
                if int(sink) in dfg_node_to_xy:
                    ...
                    canvas.create_text(
                        ...
                        font=f"Times 12",
                        fill=palette[p % len(palette)],
                        text=dataflow_hypergraph.get_node_attribute(
                            sink, 'label')
                    )
            p = p+1
...
def visualize_placement(dataflow_hypergraph, Nx, Ny, dfg_node_to_xy, output_path, energy=0, kill=True):
    ...
    MainApplication(Nx, Ny, dataflow_hypergraph, dfg_node_to_xy, output_path,
                    energy, kill, root).pack(side="top", fill="both", expand=True)
    if kill:
        root.quit()
    else:
        root.mainloop()
