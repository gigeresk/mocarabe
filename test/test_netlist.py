...
def adder_chain():
    ...
    return Netlist(dfg_path=os.path.join(HGR_ROOT, "int_adder_chain", "int_adder_chain.hgr"))


@pytest.fixture
def poly_quadratic():
    """x*x*c + 5 + 1 → y  (fanout on x, mixed IO/arith)"""
    return Netlist(dfg_path=os.path.join(HGR_ROOT, "int_poly_quadratic", "int_poly_quadratic.hgr"))
...
def test_node_labels(adder_chain):
    labels = {adder_chain.get_node_attribute(n, "label")
...
def test_topo_precedence_holds(adder_chain):
    ...
    nodes = {adder_chain.get_node_attribute(n, "label"): n
             for n in adder_chain.get_node_set()}
...
def test_to_graph_fanout(poly_quadratic):
    ...
    nodes = {poly_quadratic.get_node_attribute(n, "label"): n
